...
namespace Application.Activities.Commands.Validators
{
    public class BaseActivityValidator<T, TDto>: AbstractValidator<T> where TDto : BaseActivityDto
    {
        public BaseActivityValidator(Func<T, TDto> selector)
        {
            ...
            RuleFor(x => selector(x).Category).NotEmpty().WithMessage("Category required");
            RuleFor(x => selector(x).City).NotEmpty().WithMessage("City required");
            RuleFor(x => selector(x).Venue).NotEmpty().WithMessage("Venue required");
